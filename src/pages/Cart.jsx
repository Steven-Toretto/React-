...
function Cart({ cart, setCart }) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      {cart.length === 0 ? (
        // EMPTY CART VIEW
        <div className=" flex flex-col items-center justify-center gap-4 bg-gray-200 p-10">
          <Ban className="w-12 h-12 text-red-500" />
          <p className="text-lg font-semibold">No items in cart</p>
          <button className="bg-teal-400 text-white px-6 py-2 rounded">
            Shop Now
          </button>
        </div>
      ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="border rounded-lg p-6 bg-gray-50 h-fit">
            ...
            <div className="flex gap-2 mb-4">
              ...
              />
              <button className="bg-teal-400 text-white px-4 rounded">
                Apply
              </button>
            </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span>Subtotal</span>
                <span>
                  KSh {cartCopy.reduce((acc, curr) => acc + curr.price, 0)}
                </span>
              </div>
              ...
              <div className="flex justify-between font-semibold text-lg">
                <span>Total</span>
                <span>
                  KSh {cartCopy.reduce((acc, curr) => acc + curr.price, 450)}
                </span>
              </div>
            </div>
